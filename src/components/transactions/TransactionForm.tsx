...
export default function TransactionForm({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const finalDate = date ? formatDateToISO(date) : formatDateToISO(new Date());
  ...
  const categoriesToShow =
    selectedType === "Revenue" ? [...revenue, ...customRevenue] : [...expense, ...customExpense];
  ...
  return (
    <div className="glass-card p-6">
      <div className="flex items-center justify-between mb-6">
        <h2
          className={`text-xl font-bold flex items-center gap-2 ${isDark ? "text-[rgb(187,225,250)]" : "text-gray-900"}`}
        >
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div>
          <label
            className={`block text-sm font-medium mb-2 ${isDark ? "text-[rgb(187,225,250)]" : "text-gray-700"}`}
          >
            Descrição
          </label>
          <input
            ...
            <CustomSelect
              ...
              onChange={(value) => {
                if (value !== "customCategory") {
                  setSelectedCategory(value as ExpenseCategory | RevenueCategory);
              ...
              <div className="grid grid-cols-[1fr_auto] gap-2 items-start">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    const formatedCustomCategory =
                      customCategoryText.charAt(0).toUpperCase() +
                      customCategoryText.slice(1).toLowerCase();
                    if ((categoriesToShow as string[]).includes(formatedCustomCategory)) {
                      setErrors((prev) => ({ ...prev, duplicateCategory: true }));
                      return;
                    }
                    addCategory(
                      formatedCustomCategory as ExpenseCategory | RevenueCategory,
                      selectedType as TypeTransaction,
                    );
                    setShowCustomInput(false);
                    setSelectedCategory(
                      formatedCustomCategory as ExpenseCategory | RevenueCategory,
        ...
        <div>
          ...
          <DatePicker
            ...
            className="bg-[rgba(var(--primary-900),0.5)] border-1px border-solid border-[rgba(var(--primary-600), 0.3)] text-sm"
          />
        </div>
