            ...
            {sortedDates.map((dateKey) => (
                <div key={dateKey}>
                    <div className="flex items-center gap-3 mb-3">
                        <h3 className={`text-sm font-bold whitespace-nowrap ${isDark ? 'text-[rgb(187,225,250)]' : 'text-gray-900'}`}>
                            {formatDateHeader(groupedTransactions[dateKey][0].date)}
                        </h3>
                        <div className={`h-px flex-1 ${isDark ? 'bg-[rgb(50,130,184)]/30' : 'bg-gray-300'}`}></div>
                    </div>
