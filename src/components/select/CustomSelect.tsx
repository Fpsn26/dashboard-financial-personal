  ...
  return (
    <div ref={containerRef} className={`relative ${className}`}>
      <button
        ...
        className={`input-styled w-full flex items-center justify-between gap-2 ${
          error ? "border-red-500 border-2" : ""
        } ${disabled ? "opacity-50 cursor-not-allowed" : "cursor-pointer"}`}
      >
        <span
          className={
            !selectedOption ? (isDark ? "text-[rgb(187,225,250)]/40" : "text-gray-400") : ""
          }
        >
          {displayText}
        </span>
      ...
      {isOpen && (
        <div
          className={`absolute top-full left-0 right-0 mt-1 rounded-lg border shadow-lg z-50 overflow-hidden ${
            isDark ? "bg-[rgb(15,76,117)] border-[rgb(50,130,184)]/40" : "bg-white border-gray-300"
          }`}
        >
