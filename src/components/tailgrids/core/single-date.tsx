...
export function DatePicker({
  ...
  const togglePicker = () => {
    if (pickerRef.current) {
      ...
      setDropdownPosition({
        top: bottomSpace > 300 ? rect.bottom + 4 : rect.top - 230,
        left: rect.left,
      });
    }
  ...
  return (
    <div ref={pickerRef} className={`relative w-full ${className}`}>
      <button
        ...
        className={`input-styled flex items-center justify-between w-full text-left font-normal px-3 ${
          !selectedDate && (isDark ? "text-[rgb(187,225,250)]/40" : "text-gray-400")
        }`}
      >
      ...
      {isOpen &&
        ...
        createPortal(
          ...
          >
            <div className="p-2">
              <div className="mb-2 flex items-center justify-between">
                <button
                  ...
                  className={`p-0.5 rounded hover:bg-white/10 ${isDark ? "text-white" : "text-gray-600"}`}
                >
                  <ChevronLeft size={14} />
                </button>
                <h2
                  className={`text-[10px] font-bold uppercase ${isDark ? "text-[rgb(187,225,250)]" : "text-gray-900"}`}
                >
              ...
              <div className="grid grid-cols-7 gap-0.5 text-center mb-1">
                {["D", "S", "T", "Q", "Q", "S", "S"].map((d, index) => (
                  <span
                    key={index}
                    className={`text-[8px] font-bold uppercase ${isDark ? "text-[rgb(187,225,250)]/40" : "text-gray-400"}`}
                  >
                    {d}
                  </span>
                ))}
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        isToday(day) &&
                          !isSel &&
                          (isDark
                            ? "border border-[rgb(50,130,184)] text-[rgb(50,130,184)]"
            ...
            <div
              className={`flex gap-2 p-1.5 border-t ${isDark ? "border-[rgb(50,130,184)]/20 bg-[rgb(15,76,117)]/10" : "border-gray-100 bg-gray-50"}`}
            >
              <button
                onClick={() => setIsOpen(false)}
                className={`flex-1 py-1 text-[9px] font-bold uppercase rounded hover:bg-white/5 ${isDark ? "text-[rgb(187,225,250)]/60" : "text-gray-500"}`}
              >
                Cancelar
              </button>
              <button
                onClick={handleApply}
                className="btn-primary flex-1 py-1 text-[9px] h-auto min-h-0"
              >
                Aplicar
              </button>
            </div>
